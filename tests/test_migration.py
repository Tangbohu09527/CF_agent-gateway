...
from sqlalchemy import Engine, Integer, inspect, text
...
RESPONSE_DELIVERY_SCHEMA_TABLES = (
    ARTIFACT_SCHEMA_TABLES | ROUTING_TABLES | WORKER_TABLES | RESPONSE_DELIVERY_TABLES
)
CONTEXT_SNAPSHOT_TABLES = frozenset({"context_snapshots"})
DISPATCH_RECOVERY_TABLES = frozenset({"hermes_dispatch_recovery_audits"})
SCHEMA_TABLES = RESPONSE_DELIVERY_SCHEMA_TABLES | CONTEXT_SNAPSHOT_TABLES | DISPATCH_RECOVERY_TABLES
...
CONTEXT_SNAPSHOT_REVISION = "20260810_01"
CHECKPOINT_REVISION = "20260823_01"
DISPATCH_RECOVERY_REVISION = "20260823_02"
...
def _head_revision() -> str:
    ...
    assert head_revision == DISPATCH_RECOVERY_REVISION
...
def test_postgresql_offline_upgrade_emits_complete_schema_ddl() -> None:
    ...
    assert "ix_hermes_dispatch_context_timeline" in rendered_sql
    ...
    assert "create table context_snapshots" in rendered_sql
    assert "ck_context_snapshot_positive_version" in rendered_sql
    assert "ck_context_snapshot_nonempty_summary" in rendered_sql
    assert "ck_context_snapshot_positive_covered_until" in rendered_sql
    ...
    assert "foreign key(thread_id) references ai_threads (id) on delete restrict" in normalized_sql
...
def test_sqlite_online_upgrade_applies_batch_migration(tmp_path: Path) -> None:
    ...
    try:
        ...
        dispatch_index_columns = {
            ...
            for index in inspector.get_indexes("hermes_dispatch_records")
        }
        assert set(dispatch_index_columns) >= {
        ...
        assert dispatch_index_columns["ix_hermes_dispatch_context_timeline"] == [
            "ai_thread_id",
            "status",
            "id",
        ]
        ...
        assert CONTEXT_SNAPSHOT_TABLES.issubset(inspector.get_table_names())
        snapshot_column_types = {
            column["name"]: column["type"] for column in inspector.get_columns("context_snapshots")
        }
        snapshot_columns = set(snapshot_column_types)
        ...
        snapshot_foreign_keys = inspector.get_foreign_keys("context_snapshots")
        assert len(snapshot_foreign_keys) == 1
        assert snapshot_foreign_keys[0]["constrained_columns"] == ["thread_id"]
        assert snapshot_foreign_keys[0]["referred_table"] == "ai_threads"
        assert snapshot_foreign_keys[0]["referred_columns"] == ["id"]
        with engine.connect() as connection:
            triggers = set(
...
def test_context_snapshot_downgrade_preserves_response_delivery_schema() -> None:
    engine = create_database_engine("sqlite+pysqlite:///:memory:")
    try:
        migration.upgrade_database(engine)
        migration_config = migration.create_migration_config()
        with engine.connect() as connection:
            migration_config.attributes["connection"] = connection
            command.downgrade(migration_config, RESPONSE_DELIVERY_REVISION)

        downgraded_indexes = {
            index["name"] for index in inspect(engine).get_indexes("hermes_dispatch_records")
        }
        assert "ix_hermes_dispatch_context_timeline" not in downgraded_indexes
        assert set(inspect(engine).get_table_names()) == RESPONSE_DELIVERY_SCHEMA_TABLES | {
            "alembic_version"
        }
        assert migration.get_schema_version(engine) == RESPONSE_DELIVERY_REVISION
    finally:
        engine.dispose()
