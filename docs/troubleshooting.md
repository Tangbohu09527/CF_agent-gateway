...
Keep the operator out of the Docker group. Begin one interactive administrative session
with `sudo -v`, then use only `sudo -n` for privileged commands. The protected
env-file is passed explicitly and is never sourced or printed.
...
| Hermes Dispatch running too long | Lease timestamp and `stale_running`; Dispatch heartbeat; protected logs; external Hermes evidence | Active bounded call; stale lease after Worker loss; possible external effect | An expired running record becomes `uncertain`; preserve external evidence and use explicit recovery after review | One claim token owns the terminal write; attempt count and status transition are consistent | Do not clear the lease, reuse a claim token, or manually invoke Hermes |
...
| agent-wechat logged out | `wechat_auth`; Controller status; external owner's session check | CFserver/Debian reboot or `agent-wechat` recreation requires a new active Session; Gateway-only deployment can retain an untouched active Session; AI/Hermes host-only reboot does not require QR | After CFserver reboot/recreation, run formal Controller stop and require `{"stopped":true}`, then complete fresh QR, validate the Token File, and open the gate through Controller | `wechat_auth: logged_in`; Controller ready; controlled Message processed once | Do not assume the gate or Session state after CFserver reboot, leave Poll/Delivery running during fresh QR, expose QR/account/session data, or loosen Token File permissions |
